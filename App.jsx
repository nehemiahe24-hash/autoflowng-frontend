            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:`linear-gradient(135deg,rgba(0,200,150,0.06),rgba(56,189,248,0.04))`,border:`1px solid rgba(0,200,150,0.2)`,borderRadius:18,padding:isMobile?"18px":"22px 26px",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:15,color:C.sub}}>Security layers are active and monitoring your account.</div>
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fit,minmax(280px,1fr))",gap:12}}>
                {[
                  // FIX: Removed false AES-256 claim, replaced with accurate description
                  ...
                  // FIX: Removed false CSRF protection claim — replaced with accurate session security description
                  {icon:"🔒",t:"Secure Sessions",d:"Session tokens expire and rotate regularly to prevent unauthorized access to your account.",c:C.violet},
                  // FIX: Downgraded "AI Fraud Detection" to accurate "Anomaly Detection (basic rate limiting)"
                  {icon:"👁",t:"Anomaly Detection",d:"Basic rate limiting and pattern monitoring flag suspicious login locations and abnormal referral activity.",c:C.rose},
...
function LoginModal({ onClose, onSuccess }) {
  ...
  const [loading, setLoading] = useState(false);
  const [statusMsg, setStatusMsg] = useState(""); // live feedback during cold-start

  // ── Fetch with timeout helper ──────────────────────────────────
  const fetchWithTimeout = (url, opts = {}, ms = 55000) => {
  ...
  // ── Wake the Render server, polling until it responds ─────────
  const wakeServer = async () => {
    const PING_URL = `${BACKEND_URL}/api/ping`;
    const DEADLINE = Date.now() + 60000; // try for up to 60 s
    let attempt = 0;
    while (Date.now() < DEADLINE) {
      attempt++;
      try {
        const res = await fetchWithTimeout(PING_URL, {}, 8000);
        if (res.ok) return true; // server is awake
      } catch (_) {}
      // Update the UI so the user sees progress
      const remaining = Math.ceil((DEADLINE - Date.now()) / 1000);
      setStatusMsg(`Starting server… (${remaining}s)`);
      await new Promise(r => setTimeout(r, 2500)); // wait 2.5 s before next ping
    }
    return false; // could not wake within 60 s
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!awake) {
      setError("Server is taking too long to respond. Please wait 30 seconds and try again.");
      setStatusMsg("");
      setLoading(false);
      return;
    }
  ...
  const btnLabel = () => {
    ...
    return mode === "register" ? "Create Free Account →" : "Sign In →";
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(4,6,15,0.95)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:"20px"}}>
      <div style={{background:"#0C1120",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"36px 28px",maxWidth:420,width:"100%"}}>
        ...
        {/* Live status banner — shows during server wake-up */}
        {statusMsg && !error && (
          <div style={{background:"rgba(0,200,150,0.08)",border:"1px solid rgba(0,200,150,0.25)",borderRadius:10,padding:"10px 14px",color:"#00C896",fontSize:14,marginBottom:16,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
            <span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:"#00C896",animation:"glw 1.2s ease-in-out infinite"}}/>
            {statusMsg}
          </div>
        )}

        {error && (
          <div style={{background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:10,padding:"10px 14px",color:"#F87171",fontSize:15,marginBottom:16,textAlign:"center"}}>
            {error}
          </div>
        )}
        ...
        {mode==="register" && (
          <div style={{marginBottom:14}}>
            ...
            <input
              ...
              disabled={loading}
              style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"12px 14px",color:"#E8EEFF",fontSize:15,outline:"none",boxSizing:"border-box",opacity:loading?0.6:1}}
            />
          </div>
        )}
        ...
        <button
          ...
          disabled={loading}
          style={{width:"100%",background:loading?"rgba(0,200,150,0.5)":"#00C896",color:"#04060F",border:"none",borderRadius:12,padding:"14px",fontSize:loading?14:16,fontWeight:800,cursor:loading?"not-allowed":"pointer",marginBottom:14,fontFamily:"'Syne',sans-serif",transition:"all 0.2s"}}
        >
          {btnLabel()}
        </button>
        ...
        <div style={{textAlign:"center",marginBottom:14}}>
          ...
          <span
            onClick={()=>{ if(!loading){ setMode(mode==="register"?"login":"register"); setError(""); setStatusMsg(""); }}}
            style={{color:"#00C896",fontSize:15,cursor:loading?"default":"pointer",fontWeight:600,opacity:loading?0.4:1}}
          >
        ...
        <button
          ...
          disabled={loading}
          style={{width:"100%",background:"transparent",border:"1px solid rgba(255,255,255,0.08)",color:"rgba(232,238,255,0.4)",borderRadius:10,padding:"10px",fontSize:15,cursor:loading?"not-allowed":"pointer",opacity:loading?0.4:1}}
        >
          Cancel
        </button>
      </div>

        ...
        <div style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:12 }}>
          ...
          {messages.map((m,i) => (
            <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start", animation:"slideDown 0.2s ease" }}>
              <div style={{ maxWidth:"84%", background:m.isError?"rgba(251,113,133,0.08)":m.role==="user"?"rgba(0,200,150,0.12)":"rgba(255,255,255,0.04)", border:`1px solid ${m.isError?"rgba(251,113,133,0.3)":m.role==="user"?"rgba(0,200,150,0.25)":C.border}`, borderRadius:m.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", padding:"11px 15px", fontSize:15, lineHeight:1.65, color:m.isError?C.rose:m.role==="user"?C.green:C.ink, whiteSpace:"pre-wrap" }}>
                {m.text}
              </div>
            </div>
          ))}

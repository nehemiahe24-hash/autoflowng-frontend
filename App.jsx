...
const API_BASE = import.meta.env.VITE_API_URL || "https://autoflowng-backend.onrender.com";
...
function useServerStatus() {
  ...
  const onBackRef = useRef(null); // callback to fire when server recovers
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        setLatency(ms);
        setStatus(prev => {
          // Fire the recovery callback exactly once when transitioning to online
  ...
  // Switch to 5 s polling while watching, back to 30 s when done
  useEffect(() => {
    startPolling(watching ? 5_000 : 30_000);
  }, [watching, startPolling]);

  /**
   * Call this to register a one-shot callback fired when the server
   * recovers. Automatically switches to fast-poll mode.
   */
  const notifyWhenOnline = useCallback((cb) => {
    onBackRef.current = cb;
    setWatching(true);
    check(); // immediate check so users don't wait up to 5 s
  }, [check]);
...
function GlobalTimeClock({ isMobile }) {
  useLiveTick();
  const homeId = detectHomeZone();
  const required = ["lagos","accra","ny"];
  const zoneIds = homeId && !required.includes(homeId)
    ? [homeId, ...required].slice(0, isMobile ? 3 : 4)
    : (isMobile ? required.slice(0,2) : required);
  const zones = TIME_ZONES.filter(z => zoneIds.includes(z.id));

  return (
    <div style={{ background:"rgba(0,0,0,0.22)", borderBottom:`1px solid rgba(255,255,255,0.05)`, padding:isMobile?"5px 16px":"6px 28px", display:"flex", alignItems:"center", gap:isMobile?12:22, overflowX:"auto", flexShrink:0 }}>
      ...
      {zones.map(zone => {
        ...
        return (
          <div key={zone.id} style={{ display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
            <span style={{ fontSize:12 }}>{zone.flag}</span>
            <div>
              <div style={{ fontSize:9, color:"rgba(232,238,255,0.35)", fontFamily:"'DM Mono',monospace", letterSpacing:"0.04em", lineHeight:1, marginBottom:1 }}>{zone.label}{offset?` · ${offset}`:""}</div>
              <div style={{ fontSize:isHome?13:12, fontWeight:900, fontFamily:"'DM Mono',monospace", color:isHome?"#00C896":"rgba(232,238,255,0.82)", lineHeight:1.2 }}>{fmtLiveTime(zone.tz)}</div>
            </div>
          </div>
        );
      })}
...
function AutomationsPage({ isMobile, connected, onConnect, toast }) {
  ...
  async function runNow(id) {
    const t = TEMPLATES.find(x => x.id === id);
    if (!activeMap[id]) { toast("Enable this automation first."); return; }
    setRunning(id);
    try {
      const tok = localStorage.getItem("autoflowng_token") || "";
      ...
      if (res.ok) {
        ...
        toast(d.error || "Could not trigger automation.");
      }
...
function AgentPage({ isMobile, connected, onConnect }) {
  ...
  const SUGGESTIONS = [
    { icon:"💬", text:"Reply to all unread WhatsApp messages with a professional response" },
    { icon:"📅", text:"Schedule 5 Instagram posts for this week at peak engagement times" },
    { icon:"💰", text:"Send payment reminders to all clients with unpaid invoices" },
    { icon:"📧", text:"Follow up on all emails that haven't gotten a reply in 48 hours" },
    { icon:"🎯", text:"Capture leads from my Instagram DMs and add them to my contact list" },
    { icon:"📊", text:"Generate a performance report for my last 7 days of activity" },
  ];
  ...
  async function send(text) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/chat`, {
        ...
        headers:{ "Content-Type":"application/json", "Authorization":`Bearer ${localStorage.getItem("autoflowng_token")||""}` },
        body: JSON.stringify({
          system:`You are AutoFlow AI Agent — an intelligent, proactive automation assistant for a professional business automation SaaS platform. Your role is to help Nigerian and African business owners automate tasks across Gmail, WhatsApp, Instagram, Twitter/X, Facebook, LinkedIn, TikTok, Telegram, Slack, and Outlook.\n\nWhen a user describes a task:\n1. Confirm exactly what you will do and which platform(s) you'll use\n2. Give a clear, numbered step-by-step plan\n3. Mention any requirements (connected platforms, permissions)\n4. Estimate how many actions will be executed\n5. End with a confidence statement\n\nTone: warm, professional, confident. Use emojis naturally. Be specific — never vague. Max 6 sentences unless a detailed plan is requested. Format structured responses with bullet points or numbered steps.`,
      ...
      const detectedPlatform = platforms.find(p => msg.toLowerCase().includes(p)) || platforms[0] || "gmail";
      const statusFlow = ["queued","running","done"];
      const newTask = {
        id:Date.now(),
      ...
      setTimeout(() => {
        setTasks(t => t.map(x => x.id===newTask.id ? {...x, status:"done", done:x.total} : x));
      }, 4000 + Math.random()*3000);
    } catch {
      setMessages(m => [...m, { role:"assistant", text:"I had a connection issue. Please check your internet and try again. 💪" }]);
    }
    setLoading(false);
  }
...
function KnowledgePage({ isMobile }) {
  const [messages, setMessages] = useState([
    { role:"assistant", text:"Hello! 👋 I'm your AutoFlow Knowledge Assistant. I can help with business, real estate, finance, engineering, education, and much more — in multiple languages.\n\nPick a topic on the left or ask me anything!" }
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const QUICK_QUESTIONS = {
    realestate: ["How much does land cost in Lekki Lagos?", "What documents do I need to buy land in Nigeria?", "How do I verify land ownership in Abuja?", "What is C of O and why is it important?", "How do I find land for sale in my state?"],
    business_ng: ["How do I register a business with CAC?", "What taxes does my small business need to pay?", "How do I get a CBN/BOI loan for my business?", "How do I open a corporate bank account in Nigeria?", "What is the best business to start in Nigeria with ₦500k?"],
...
function LoginModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    // FIX: Ping the server first to wake it up from Render free-tier sleep.
    // Cold starts can take 30-90s. Without this, the first login after any idle
    // period would fail with "Could not connect" because the 15s timeout fired
    // before the server finished waking up.
    try {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (attempt < MAX_ATTEMPTS) {
          // First attempt timed out — server may still be cold-starting. Retry once.
        ...
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          {srvStatus === "waking" && (
            <p style={{color:"#FBBF24",fontSize:12,marginTop:8,marginBottom:0,lineHeight:1.5}}>
              Server is waking up — sign-in will work in ~30 seconds.
            </p>
          )}
          {srvStatus === "offline" && (
            <p style={{color:"#FB7185",fontSize:12,marginTop:8,marginBottom:0,lineHeight:1.5}}>
              Server appears offline. Please wait a moment before signing in.
            </p>
          )}
          {(srvStatus === "waking" || srvStatus === "offline") && (
            <ServerNotifyButton
...
function AdminDashboard({ onExit }) {
  ...
  async function sendAdminAI() {
    ...
    try {
      ...
      setAiMsgs(m=>[...m,{ role:"assistant", text: d.content?.[0]?.text||d.error||"No response." }]);
    } catch { setAiMsgs(m=>[...m,{ role:"assistant", text:"Connection error — check server." }]); }
    setAiLoading(false);
  }
  ...
  function SystemTab() {
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
          <AdminKPI icon="⏱" label="Uptime" value={`${upH}h ${upM}m`} color={C.green}/>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 130px)", background:C.s1, border:`1px solid ${C.border}`, borderRadius:18, overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:16, display:"flex", flexDirection:"column", gap:10 }}>
          {aiMsgs.map((m,i)=>(
            <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start", animation:"slideDown 0.2s ease" }}>
              <div style={{ maxWidth:"87%", background:m.role==="user"?"rgba(167,139,250,0.15)":"rgba(255,255,255,0.04)", border:`1px solid ${m.role==="user"?"rgba(167,139,250,0.3)":C.border}`, borderRadius:m.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", padding:"11px 15px", fontSize:14, lineHeight:1.7, color:m.role==="user"?"#A78BFA":C.ink, whiteSpace:"pre-wrap" }}>
                {m.text}
              </div>
            </div>
          ))}
    ...
    <div style={{ display:"flex", height:"100vh", background:C.bg, color:C.ink, fontFamily:"'DM Sans',sans-serif", overflow:"hidden" }}>
      ...
      <div style={{ width:224, background:C.s1, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", flexShrink:0 }}>
        <div style={{ padding:"20px 18px", borderBottom:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
            <div style={{ width:32, height:32, borderRadius:9, background:`linear-gradient(135deg,#A78BFA,${C.green})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>⚡</div>
            <div>
      ...
      <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"14px 24px", borderBottom:`1px solid ${C.border}`, background:C.s1, display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0, gap:12 }}>
          ...
          <GlobalTimeClock isMobile={false}/>
        </div>
        {/* Live clock strip */}

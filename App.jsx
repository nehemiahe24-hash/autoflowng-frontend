...
function AppDashboard({ onBack }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND}/api/referrals/balance`, { credentials: "include" })
      .then(r => r.ok ? r.json() : { balance: 0 })
      .then(d => { if (d.balance !== undefined) setRefBalance(d.balance); })
      .catch(() => setRefBalance(0));

    // Load real platform connections from backend
      ...
      .then(d => {
        if (d?.connections?.length) {
          setPlatforms(prev => prev.map(p => ({
            ...p,
            connected: d.connections.some(c => c.platform === p.id),
          })));
        }
      })
      .catch(() => {});

    // If just came back from Gmail OAuth — go to connections tab and show success
    if (localStorage.getItem("autoflowng_gmail_just_connected")) {
      localStorage.removeItem("autoflowng_gmail_just_connected");
      setNav("connections");
      setTimeout(() => toast("Gmail connected successfully! ✅"), 500);
    }
  }, []);
  ...
  function connectPlatform(id) {
    ...
    if (pl.connected) {
      ...
      if (id === "gmail") {
        // Save current session before leaving — we restore it on return
        const user = localStorage.getItem("autoflowng_user");
        if (!user) {
          // Try to get user from backend first
          fetch(`${BACKEND}/api/auth/me`, { credentials: "include" })
            .then(r => r.ok ? r.json() : null)
            .then(d => {
              if (d?.user) localStorage.setItem("autoflowng_user", JSON.stringify(d.user));
              window.location.href = `${BACKEND}/api/connect/gmail`;
            })
            .catch(() => { window.location.href = `${BACKEND}/api/connect/gmail`; });
        } else {
          window.location.href = `${BACKEND}/api/connect/gmail`;
        }
      } else if (id === "slack") {
...
export default function Root() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // Gmail OAuth just completed — restore session and go to connections
      window.history.replaceState({}, "", window.location.pathname);
      const savedUser = localStorage.getItem("autoflowng_user");
      if (savedUser) {
        localStorage.setItem("autoflowng_gmail_just_connected", "true");
        setScreen("app");
      } else {
        // Try to restore session from backend
        fetch("https://autoflowng-backend.onrender.com/api/auth/me", { credentials: "include" })
          .then(r => r.ok ? r.json() : null)
          .then(d => {
            if (d?.user) {
              localStorage.setItem("autoflowng_user", JSON.stringify(d.user));
              localStorage.setItem("autoflowng_gmail_just_connected", "true");
              setScreen("app");
            }
          })
      ...
      const savedUser = localStorage.getItem("autoflowng_user");
      if (saved || savedUser) setScreen("app");
    }
  }, []);

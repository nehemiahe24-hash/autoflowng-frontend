...
function WorkflowBuilderPage({ isMobile, connected, toast }) {
  const [view, setView] = useState("list"); // list | builder | run_history | debug | versions
    ...
    try {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (["run_started","step_started","step_completed","run_completed","run_failed","step_log","execution_filtered"].includes(evt.event)) {
            liveRef.current = [...liveRef.current.slice(-49), evt];
            setLiveEvents([...liveRef.current]);
            if (evt.event === "run_started") setLiveRunId(evt.runId);
            if (evt.event === "run_completed" || evt.event === "run_failed") {
              setTimeout(() => { loadWorkflows(); loadStats(); }, 1500);
            }
          }
        } catch(err) {}
      };
  ...
  const loadRuns = async (wfId) => {
    setRunLoading(true);
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wfId}/runs/enterprise`, { headers: hdr() });
      const d = await r.json();
      setRuns(d.runs || []);
    } catch(e) {
      // fallback to original runs endpoint
      try {
        const r2 = await fetch(`${API_BASE}/api/workflows/${wfId}/runs`, { headers: hdr() });
  ...
  const runWorkflow = async (wf) => {
    try {
      ...
      // Use enterprise run endpoint (queued + real-time)
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/run/enterprise`, { method:"POST", headers: hdr(), body: JSON.stringify({ trigger: "manual" }) });
      const d = await r.json();
      if (d.error) {
        // Fallback to original run endpoint
        await fetch(`${API_BASE}/api/workflows/${wf.id}/run`, { method:"POST", headers: hdr() });
      }
      setTimeout(() => { loadWorkflows(); loadStats(); }, 3000);
  ...
  const duplicateWorkflow = async (wf) => {
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/duplicate`, { method:"POST", headers: hdr() });
      const d = await r.json();
      if (d.workflow) {
        setWorkflows(prev => [d.workflow, ...prev]);
        toast(`Duplicated as "${d.workflow.name}" ✅`);
      }
    } catch(e) { toast("Failed to duplicate"); }
  };

  const exportWorkflow = async (wf) => {
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/export`, { headers: hdr() });
      const d = await r.json();
      const blob = new Blob([JSON.stringify(d, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = `${wf.name.replace(/\s+/g,"-")}.json`; a.click();
      URL.revokeObjectURL(url);
      toast("Workflow exported! ✅");
    } catch(e) { toast("Export failed"); }
  };

  const generateWebhookKey = async (wf) => {
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/webhook-key`, { method:"POST", headers: hdr() });
      const d = await r.json();
      if (d.webhook_url) {
        setWebhookUrl(d.webhook_url);
        setWorkflows(prev => prev.map(w => w.id === wf.id ? { ...w, trigger_type: "webhook" } : w));
        toast("Webhook URL generated! ✅");
      }
    } catch(e) { toast("Failed to generate webhook"); }
  };

  const debugWorkflow = async (wf) => {
    setDebugLoading(true);
    setDebugAnalysis(null);
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/debug`, { method:"POST", headers: hdr() });
      const d = await r.json();
      setDebugAnalysis(d.analysis || "No analysis available");
      setSelected(wf);
      setView("debug");
    } catch(e) { toast("Debug failed"); }
  ...
  const optimizeWorkflow = async (wf) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/optimize`, { method:"POST", headers: hdr() });
      const d = await r.json();
      setOptimizeSuggestions(d.suggestions || "No suggestions available");
      setSelected(wf);
      setView("debug");
    } catch(e) { toast("Optimization failed"); }
    finally { setOptimizing(false); }
  };

  const loadVersions = async (wf) => {
    try {
      const r = await fetch(`${API_BASE}/api/workflows/${wf.id}/versions`, { headers: hdr() });
      const d = await r.json();
      setVersions(d.versions || []);
      setSelected(wf);
      setView("versions");
    } catch(e) { toast("Failed to load versions"); }
  };

  const saveVersion = async (wf) => {
    try {
      await fetch(`${API_BASE}/api/workflows/${wf.id}/version`, { method:"POST", headers: hdr() });
      toast("Version snapshot saved! ✅");
    } catch(e) { toast("Failed to save version"); }
  };

  const restoreVersion = async (versionId) => {
    if (!window.confirm("Restore this version? Current state will be overwritten.")) return;
    try {
      await fetch(`${API_BASE}/api/workflows/${selected.id}/versions/${versionId}/restore`, { method:"POST", headers: hdr() });
  ...
  const checkIntegrationHealth = async () => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/integrations/health`, { headers: hdr() });
      const d = await r.json();
      setIntegrationHealth(d.health || {});
  ...
  const buildWithAI = async () => {
    ...
    setAiBuilding(true);
    try {
      const connectedIds = (connected || []).map(c => c.id || c.platform);
  ...
  const saveWorkflow = async (wfData) => {
    try {
      ...
      if (!isNew && d.workflow?.id) {
        fetch(`${API_BASE}/api/workflows/${d.workflow.id}/version`, { method:"POST", headers: hdr() }).catch(() => {});
      }
  ...
  // ── STEP TYPES catalog (enterprise-extended) ───────────────────
  const STEP_TYPES = [
    ...
    { type:"webhook_call",   icon:"🔗", label:"Webhook Call",      color:"#F97316", desc:"Call any external URL" },
    { type:"http_request",   icon:"🌐", label:"HTTP Request",      color:"#F97316", desc:"Advanced HTTP with headers/auth" },
    { type:"set_variable",   icon:"📌", label:"Set Variable",      color:"#EC4899", desc:"Store a value for later" },
    // Enterprise step types
    { type:"router",         icon:"🔀", label:"Router",            color:"#FBBF24", desc:"Multi-branch conditional routing" },
    { type:"parallel",       icon:"⚡", label:"Parallel",          color:"#38BDF8", desc:"Run multiple steps concurrently" },
    { type:"iterator",       icon:"🔁", label:"Iterator",          color:"#A78BFA", desc:"Loop over a list of items" },
    { type:"aggregator",     icon:"🗜", label:"Aggregator",        color:"#6B7280", desc:"Combine multiple outputs" },
    { type:"transform",      icon:"🔄", label:"Transform",         color:"#EC4899", desc:"Transform data: uppercase, extract, replace" },
    { type:"filter",         icon:"🚧", label:"Filter",            color:"#FBBF24", desc:"Stop execution if condition fails" },
    { type:"error_handler",  icon:"🛡", label:"Error Handler",     color:"#F87171", desc:"Handle errors with fallback logic" },
  ];
  ...
  const LiveMonitor = () => {
    ...
    return (
      <div style={{ background:"rgba(0,200,150,0.05)", border:`1px solid rgba(0,200,150,0.2)`, borderRadius:14, padding:"14px 18px", marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          <div style={{ fontWeight:800, fontSize:13, color:C.green, fontFamily:"'Syne',sans-serif" }}>
            ⚡ Live Execution {wsConnected ? <span style={{ fontSize:11, color:C.green }}>● Connected</span> : <span style={{ fontSize:11, color:C.sub }}>○ Polling</span>}
          </div>
          <button onClick={()=>{ liveRef.current=[]; setLiveEvents([]); setLiveRunId(null); }} style={{ background:"none", border:"none", color:C.sub, cursor:"pointer", fontSize:12 }}>Clear</button>
        </div>
        <div style={{ maxHeight:120, overflowY:"auto", fontFamily:"monospace", fontSize:11 }}>
          {liveEvents.slice(-20).map((ev, i) => (
            <div key={i} style={{ color: ev.event==="run_completed"?C.green : ev.event==="run_failed"?C.rose : ev.status==="error"?C.rose : ev.status==="warn"?C.amber : C.sub, marginBottom:2 }}>
              [{ev.event}] {ev.stepType ? `${ev.stepType}: ` : ""}{ev.msg || ev.workflowName || ev.status || ""}
              {ev.completedSteps !== undefined ? ` (${ev.completedSteps}/${ev.totalSteps} steps)` : ""}
            </div>
          ))}
        </div>
      </div>
  ...
  if (view === "debug") return (
    <div>
      ...
      <div style={{ fontWeight:800, fontSize:18, fontFamily:"'Syne',sans-serif", marginBottom:4 }}>
        {optimizeSuggestions ? "🚀 AI Optimization" : "🔍 AI Debugger"}: {selected?.name}
      </div>
      <div style={{ fontSize:13, color:C.sub, marginBottom:16 }}>AI analysis of your workflow structure and logic</div>
      <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:16, padding:"20px 24px" }}>
        {(debugLoading || optimizing) ? (
          <div style={{ color:C.sub, textAlign:"center", padding:32 }}>🤖 AI analyzing workflow…</div>
        ) : (
          <div style={{ color:C.ink, fontSize:14, lineHeight:1.8, whiteSpace:"pre-wrap" }}>
            {renderMarkdown(optimizeSuggestions || debugAnalysis || "No analysis available")}
          </div>
        )}
      </div>
      <div style={{ display:"flex", gap:10, marginTop:16 }}>
        <button onClick={()=>{ setDebugAnalysis(null); setOptimizeSuggestions(null); }} style={{ background:"rgba(0,0,0,0.3)", border:`1px solid ${C.border}`, color:C.sub, borderRadius:8, padding:"8px 16px", cursor:"pointer" }}>Clear</button>
        <button onClick={()=>{ if(selected) optimizeSuggestions ? debugWorkflow(selected) : optimizeWorkflow(selected); }} style={{ background:C.violet, color:"#000", border:"none", borderRadius:8, padding:"8px 18px", fontWeight:800, cursor:"pointer" }}>
          {optimizeSuggestions ? "🔍 Run Debug Instead" : "🚀 Optimize Instead"}
        </button>
      </div>
    </div>
  );
  ...
  if (view === "versions") return (
    <div>
      <button onClick={()=>setView("list")} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.sub, borderRadius:8, padding:"8px 16px", cursor:"pointer", marginBottom:20 }}>← Back to Workflows</button>
      <div style={{ fontWeight:800, fontSize:18, fontFamily:"'Syne',sans-serif", marginBottom:4 }}>📦 Version History: {selected?.name}</div>
      <div style={{ fontSize:13, color:C.sub, marginBottom:16 }}>Each save creates a version snapshot you can restore</div>
      {versions.length === 0 ? (
        <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:14, padding:"32px", textAlign:"center", color:C.sub }}>No versions saved yet — edit and save the workflow to create version snapshots</div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {versions.map(v => (
            <div key={v.id} style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ fontWeight:800, fontSize:14 }}>Version {v.version}</div>
                <div style={{ fontSize:12, color:C.sub }}>{new Date(v.created_at).toLocaleString()}</div>
              </div>
              <button onClick={()=>restoreVersion(v.id)} style={{ background:"rgba(0,200,150,0.1)", color:C.green, border:`1px solid rgba(0,200,150,0.3)`, borderRadius:8, padding:"6px 14px", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                Restore
              </button>
            </div>
          ))}
  ...
  if (view === "list") return (
    <div>
      ...
      <div style={{ marginBottom:16 }}>
        <button onClick={checkIntegrationHealth} disabled={healthLoading} style={{ background:"rgba(56,189,248,0.08)", color:C.sky, border:`1px solid rgba(56,189,248,0.25)`, borderRadius:10, padding:"8px 16px", fontSize:13, fontWeight:700, cursor:"pointer" }}>
          {healthLoading ? "Checking…" : "🩺 Check Integration Health"}
        </button>
        {integrationHealth && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginTop:10 }}>
            {Object.entries(integrationHealth).map(([platform, h]) => (
              <div key={platform} style={{ background:`${h.status==="healthy"?C.green:h.status==="error"?C.rose:C.amber}14`, border:`1px solid ${h.status==="healthy"?C.green:h.status==="error"?C.rose:C.amber}30`, borderRadius:8, padding:"4px 12px", fontSize:12, fontWeight:700, color:h.status==="healthy"?C.green:h.status==="error"?C.rose:C.amber }}>
                {platform}: {h.status} {h.latency_ms ? `(${h.latency_ms}ms)` : ""}
              </div>
            ))}
          </div>
        )}
      ...
      {/* Webhook URL display */}
      {webhookUrl && (
        <div style={{ background:"rgba(249,115,22,0.08)", border:"1px solid rgba(249,115,22,0.2)", borderRadius:12, padding:"12px 16px", marginBottom:16, fontSize:13, color:"#F97316" }}>
          🔗 Webhook URL: <code style={{ background:"rgba(0,0,0,0.3)", padding:"2px 8px", borderRadius:4, fontSize:12 }}>{webhookUrl}</code>
          <button onClick={()=>{ navigator.clipboard?.writeText(webhookUrl); toast("Copied!"); }} style={{ background:"none", border:"none", color:"#F97316", cursor:"pointer", marginLeft:8, fontSize:12 }}>📋 Copy</button>
          <button onClick={()=>setWebhookUrl(null)} style={{ background:"none", border:"none", color:C.sub, cursor:"pointer", fontSize:12 }}>✕</button>
        </div>
      )}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={()=>{ const inp = document.createElement("input"); inp.type="file"; inp.accept=".json"; inp.onchange=async e=>{ const f=e.target.files[0]; if(!f) return; const text=await f.text(); try{ const d=JSON.parse(text); const r=await fetch(`${API_BASE}/api/workflows/import`,{method:"POST",headers:hdr(),body:JSON.stringify(d)}); const res=await r.json(); if(res.workflow){setWorkflows(p=>[res.workflow,...p]);toast("Workflow imported! ✅");}else toast("Import failed: "+res.error); }catch(err){toast("Invalid JSON file");}}; inp.click(); }} style={{ background:"rgba(255,255,255,0.05)", color:C.sub, border:`1px solid ${C.border}`, borderRadius:10, padding:"9px 14px", fontWeight:700, fontSize:13, cursor:"pointer" }}>
            📥 Import
          </button>
            ...
            return (
              <div key={wf.id} style={{ background:C.s1, border:`1px solid ${wf.is_active ? "rgba(0,200,150,0.3)" : C.border}`, borderRadius:16, padding:"16px 20px", transition:"border 0.2s" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                    {wf.version > 1 && <Chip color={C.violet}>v{wf.version}</Chip>}
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  ...
                  <button onClick={()=>debugWorkflow(wf)} disabled={debugLoading} style={{ background:"rgba(167,139,250,0.08)", color:C.violet, border:`1px solid rgba(167,139,250,0.25)`, borderRadius:8, padding:"6px 14px", fontSize:13, cursor:"pointer" }}>
                    🔍 Debug
                  </button>
                  <button onClick={()=>duplicateWorkflow(wf)} style={{ background:"rgba(255,255,255,0.04)", color:C.sub, border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 12px", fontSize:12, cursor:"pointer" }}>
                    ⧉ Copy
                  </button>
                  <button onClick={()=>exportWorkflow(wf)} style={{ background:"rgba(255,255,255,0.04)", color:C.sub, border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 12px", fontSize:12, cursor:"pointer" }}>
                    📤 Export
                  </button>
                  <button onClick={()=>loadVersions(wf)} style={{ background:"rgba(255,255,255,0.04)", color:C.sub, border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 12px", fontSize:12, cursor:"pointer" }}>
                    📦 Versions
                  </button>
                  {wf.trigger_type !== "webhook" && (
                    <button onClick={()=>generateWebhookKey(wf)} style={{ background:"rgba(249,115,22,0.06)", color:"#F97316", border:`1px solid rgba(249,115,22,0.2)`, borderRadius:8, padding:"6px 12px", fontSize:12, cursor:"pointer" }}>
                      🔗 Webhook
                    </button>
                  )}
  ...
  if (view === "run_history") return (
    <div>
      ...
      {runLoading ? <div style={{ color:C.sub, padding:20 }}>Loading…</div> : runs.length === 0 ? (
        ...
        const stColor = run.status==="completed"?C.green:run.status==="failed"?C.rose:run.status==="filtered"?C.amber:C.amber;
        return (
          <div key={run.id} style={{ background:C.s1, border:`1px solid ${stColor}30`, borderRadius:14, padding:"16px 20px", marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
              <div>
                ...
                {run.trigger_type && <Chip color={C.sub}>{run.trigger_type}</Chip>}
              ...
              <div style={{ background:"rgba(0,0,0,0.3)", borderRadius:8, padding:"10px 14px", maxHeight:180, overflowY:"auto" }}>
                {logs.map((step,i) => (
                  <div key={i} style={{ marginBottom:6 }}>
                    <div style={{ fontSize:12, color:C.sub, fontFamily:"monospace" }}>Step {(step.step||i)+1}: {step.type} {step.name ? `"${step.name}"` : ""}{step.attempt > 1 ? ` (retry ${step.attempt})` : ""}</div>
                    {(step.logs||[]).map((l,j) => (
                      <div key={j} style={{ fontSize:12, color:l.status==="error"?C.rose:l.status==="warn"?C.amber:l.status==="skip"?C.sub:C.green, fontFamily:"monospace", paddingLeft:12 }}>
...
function WorkflowEditor({ workflow, onSave, onBack, STEP_TYPES, TRIGGER_TYPES, toast }) {
  ...
  const addStep = (type) => {
    ...
    const defaultConfig = {
      ...
      http_request:  { url:"", method:"POST", body:"{}", headers:{} },
      set_variable:  { key:"my_variable", value:"" },
      // Enterprise step defaults
      router:        { input:"{{last_output}}", routes:[{ condition_key:"", operator:"contains", value:"", branch:"branch_a" }], default_branch:"default" },
      parallel:      { steps:[] },
      iterator:      { array_key:"items", limit:10, steps:[], join_with:"\n" },
      aggregator:    { keys:["parallel_outputs","iterator_results"], separator:"\n---\n" },
      transform:     { input:"{{last_output}}", operation:"uppercase", store_as:"transformed_output" },
      filter:        { value:"{{last_output}}", operator:"not_empty", compare:"" },
      error_handler: { action:"notify_email", fallback_prompt:"Generate a professional fallback response" },
    }[type] || {};
  ...
  // Render config field — handles enterprise step types intelligently
  const renderConfigField = (step, idx, key, val) => {
    // Enterprise-specific fields
    if (key === "operation" && step.type === "transform") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={labelS}>Operation</label>
        <select value={val} onChange={e=>updateStepConfig(idx,key,e.target.value)} style={{ ...inputS, cursor:"pointer" }}>
          {["uppercase","lowercase","trim","truncate","replace","extract_emails","extract_urls","word_count","json_parse","template"].map(o=><option key={o} value={o}>{o}</option>)}
        </select>
      </div>
    );
    if (key === "operator") return (
      <div key={key} style={{ marginBottom:10 }}>
        ...
        <select value={val} onChange={e=>updateStepConfig(idx,key,e.target.value)} style={{ ...inputS, cursor:"pointer" }}>
          {["contains","not_contains","equals","not_empty","greater_than","starts_with","regex"].map(o=><option key={o} value={o}>{o}</option>)}
        </select>
      </div>
    );
    if (key === "action" && step.type === "error_handler") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={labelS}>On Error Action</label>
        <select value={val} onChange={e=>updateStepConfig(idx,key,e.target.value)} style={{ ...inputS, cursor:"pointer" }}>
          {[["log","Log only"],["notify_email","Notify via Email"],["notify_telegram","Notify via Telegram"],["fallback_ai","AI Fallback Response"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}
        </select>
      </div>
    );
    if (key === "method") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={labelS}>Method</label>
        <select value={val} onChange={e=>updateStepConfig(idx,key,e.target.value)} style={{ ...inputS, cursor:"pointer" }}>
          {["POST","GET","PUT","PATCH","DELETE"].map(m=><option key={m} value={m}>{m}</option>)}
        </select>
      </div>
    );
    if (key === "seconds" || key === "max_tokens" || key === "limit") return (
      <div key={key} style={{ marginBottom:10 }}>
    ...
    if (key === "routes" && step.type === "router") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={labelS}>Routes (conditions → branches)</label>
        <div style={{ background:"rgba(0,0,0,0.2)", borderRadius:8, padding:10, fontSize:12, color:C.sub, fontFamily:"monospace" }}>
          {(Array.isArray(val) ? val : []).map((r,ri) => (
            <div key={ri} style={{ display:"flex", gap:6, marginBottom:6, alignItems:"center" }}>
              <input placeholder="key" value={r.condition_key||""} onChange={e=>{ const nr=[...val]; nr[ri]={...nr[ri],condition_key:e.target.value}; updateStepConfig(idx,key,nr); }} style={{ ...inputS, padding:"6px 8px", fontSize:11, width:90 }} />
              <select value={r.operator||"contains"} onChange={e=>{ const nr=[...val]; nr[ri]={...nr[ri],operator:e.target.value}; updateStepConfig(idx,key,nr); }} style={{ ...inputS, padding:"6px 8px", fontSize:11, width:90, cursor:"pointer" }}>
                {["contains","equals","not_empty","greater_than","regex"].map(o=><option key={o} value={o}>{o}</option>)}
              </select>
              <input placeholder="value" value={r.value||""} onChange={e=>{ const nr=[...val]; nr[ri]={...nr[ri],value:e.target.value}; updateStepConfig(idx,key,nr); }} style={{ ...inputS, padding:"6px 8px", fontSize:11, width:80 }} />
              <span style={{ color:C.sub }}>→</span>
              <input placeholder="branch" value={r.branch||""} onChange={e=>{ const nr=[...val]; nr[ri]={...nr[ri],branch:e.target.value}; updateStepConfig(idx,key,nr); }} style={{ ...inputS, padding:"6px 8px", fontSize:11, width:80, color:"#FBBF24" }} />
              <button onClick={()=>{ const nr=val.filter((_,i)=>i!==ri); updateStepConfig(idx,key,nr); }} style={{ background:"none", border:"none", color:C.rose, cursor:"pointer", fontSize:14 }}>✕</button>
            </div>
          ))}
          <button onClick={()=>updateStepConfig(idx,key,[...(val||[]),{ condition_key:"", operator:"contains", value:"", branch:`branch_${(val||[]).length+1}` }])} style={{ background:"rgba(251,191,36,0.1)", border:"1px solid rgba(251,191,36,0.3)", borderRadius:6, padding:"4px 10px", color:"#FBBF24", cursor:"pointer", fontSize:11 }}>+ Add Route</button>
        </div>
      </div>
    );
    if (key === "keys" && step.type === "aggregator") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={labelS}>Keys to aggregate (comma-separated)</label>
        <input value={Array.isArray(val)?val.join(", "):val} onChange={e=>updateStepConfig(idx,key,e.target.value.split(",").map(s=>s.trim()))} style={inputS} placeholder="parallel_outputs, iterator_results" />
      </div>
    );
    if ((key === "steps" || key === "body") && typeof val === "object") return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={{ ...labelS, textTransform:"capitalize" }}>{key.replace(/_/g," ")} (JSON)</label>
        <textarea value={JSON.stringify(val,null,2)} onChange={e=>{ try{updateStepConfig(idx,key,JSON.parse(e.target.value))}catch(_){} }} rows={3} style={{ ...inputS, resize:"vertical", fontFamily:"monospace", fontSize:11 }} />
      </div>
    );
    // Skip internal-only fields
    if (key === "routes" || key === "headers") {
      if (key === "headers") return (
        <div key={key} style={{ marginBottom:10 }}>
          <label style={labelS}>Headers (JSON)</label>
          <textarea value={JSON.stringify(val||{},null,2)} onChange={e=>{ try{updateStepConfig(idx,key,JSON.parse(e.target.value))}catch(_){} }} rows={2} style={{ ...inputS, resize:"vertical", fontFamily:"monospace", fontSize:11 }} />
        </div>
      );
      return null;
    }
    ...
    return (
      <div key={key} style={{ marginBottom:10 }}>
        <label style={{ ...labelS, textTransform:"capitalize" }}>{key.replace(/_/g," ")}</label>
        <textarea value={val} onChange={e=>updateStepConfig(idx,key,e.target.value)} rows={key==="prompt"||key==="body"||key==="text"||key==="template"?3:1} style={{ ...inputS, resize:key==="prompt"||key==="body"||key==="text"||key==="template"?"vertical":"none" }} placeholder={key==="prompt"?"Write a post about {{topic}}…":key==="body"?"{{ai_output}}":""} />
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:16, padding:"20px 24px", marginBottom:16 }}>
        ...
        {triggerType === "webhook" && (
          <div style={{ background:"rgba(249,115,22,0.08)", border:"1px solid rgba(249,115,22,0.2)", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#F97316" }}>
            🔗 Webhook URL: <code style={{ background:"rgba(0,0,0,0.3)", padding:"2px 8px", borderRadius:4 }}>
              {triggerConfig.webhook_key
                ? `${API_BASE}/api/webhooks/workflow/${triggerConfig.webhook_key}`
                : `Save workflow first, then click "🔗 Webhook" on the workflow card to generate a URL`}
            </code>
            <br/><span style={{ fontSize:11 }}>POST any JSON payload to this URL to trigger the workflow. Payload is available as <code>{"{{webhook_body}}"}</code>.</span>
          </div>
        )}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:16 }}>
        {steps.map((step, idx) => {
          const st = STEP_TYPES.find(s => s.type === step.type);
          const isEnterprise = ["router","parallel","iterator","aggregator","transform","filter","error_handler","http_request"].includes(step.type);
          return (
            <div key={step.id||idx} style={{ background:C.s1, border:`1px solid ${isEnterprise ? (st?.color||C.border)+"40" : (st?.color||C.border)+"20"}`, borderRadius:14, overflow:"hidden" }}>
              {/* Step header */}
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 16px", borderBottom:`1px solid ${C.border}` }}>
                ...
                <div style={{ flex:1, display:"flex", gap:8, alignItems:"center" }}>
                  ...
                  {isEnterprise && <span style={{ fontSize:10, background:`${st?.color||"#666"}20`, color:st?.color||"#666", borderRadius:4, padding:"1px 6px", fontWeight:700 }}>ENTERPRISE</span>}
              ...
              <div style={{ padding:"12px 16px" }}>
                {Object.entries(step.config||{}).map(([key, val]) => renderConfigField(step, idx, key, val))}
                {/* Retry config for enterprise steps */}
                {isEnterprise && (
                  <div style={{ marginTop:8, paddingTop:8, borderTop:`1px solid ${C.border}` }}>
                    <label style={labelS}>Max Retries (on failure)</label>
                    <input type="number" min="0" max="5" value={step.config?.max_retries||0} onChange={e=>updateStepConfig(idx,"max_retries",parseInt(e.target.value)||0)} style={{ ...inputS, width:80 }} />
                  </div>
                )}
                {step.type!=="delay" && step.type!=="set_variable" && step.type!=="router" && step.type!=="filter" && step.type!=="error_handler" && (
                  <div style={{ fontSize:11, color:C.sub, marginTop:4 }}>
                    💡 Use <code style={{ background:"rgba(255,255,255,0.08)", padding:"1px 5px", borderRadius:3 }}>{`{{ai_output}}`}</code>, <code style={{ background:"rgba(255,255,255,0.08)", padding:"1px 5px", borderRadius:3 }}>{`{{user_name}}`}</code>, <code style={{ background:"rgba(255,255,255,0.08)", padding:"1px 5px", borderRadius:3 }}>{`{{timestamp}}`}</code>, <code style={{ background:"rgba(255,255,255,0.08)", padding:"1px 5px", borderRadius:3 }}>{`{{last_output}}`}</code> as dynamic values
                  </div>
                )}
              </div>
            </div>
      ...
      {addingStep ? (
        <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 20px" }}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:4 }}>Choose step type:</div>
          <div style={{ fontSize:12, color:C.sub, marginBottom:12 }}>Core steps + 7 enterprise step types (Router, Parallel, Iterator, Aggregator, Transform, Filter, Error Handler)</div>

...
function PayModal({ plan, onClose, onSuccess }) {
  ...
  function pay() {
    ...
    const handler = window.PaystackPop.setup({
      key: import.meta.env.VITE_PAYSTACK_PUBLIC_KEY || "pk_live_REPLACE_WITH_YOUR_KEY",
      email: email,
...
function AppDashboard({ onBack }) {
  ...
  useEffect(() => {
    ...
    // Get referral data
    fetch(`${BACKEND}/api/referrals`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (d) {
    ...
    // Get payment history
    fetch(`${BACKEND}/api/payments/history`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => {
    ...
    fetch(`${BACKEND}/api/connections`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (d?.connections) {
          const connectedIds = d.connections;
          setPlatforms(prev => prev.map(p => ({...p, connected: connectedIds.includes(p.id)})));
          setConnectedCount(connectedIds.length);
        }
      }).catch(()=>{});

    // Get user stats
    fetch(`${BACKEND}/api/user/stats`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (d?.user) {
          setUsedActions(d.actionsUsed || 0);
        }
      }).catch(()=>{});
  }, []);
  ...
  const user = { name: userName, plan: userPlan !== "trial" ? userPlan : (trial.subscribed ? "Business" : "Free Trial"), code: userCode };
  ...
  function connectPlatform(id) {
    ...
    if (pl.connected) {
      // Disconnect
      setConnecting(id);
      fetch(`${BACKEND}/api/connections/${id}`, { method:"DELETE", credentials:"include" })
        .then(()=>{
      ...
      // Connect — redirect to backend OAuth or show instructions
      if (id === "gmail") {
        window.location.href = `${BACKEND}/api/connect/gmail`;
      } else if (id === "slack") {
        window.location.href = `${BACKEND}/api/connect/slack`;
      } else {
        // For other platforms - show coming soon toast
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={{background:`linear-gradient(135deg,rgba(0,200,150,0.07),rgba(14,165,233,0.04))`,border:`1px solid rgba(0,200,150,0.2)`,borderRadius:20,padding:isMobile?"20px":"28px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:22}}>
                  {[[refStats.total.toString(),"Total Referrals"],[refStats.successful.toString(),"Successful"],[`₦${refStats.totalEarned.toLocaleString()}`,"Total Earned"]].map(([v,l])=>(
              ...
              <div style={{background:C.s1,border:`1px solid ${C.border}`,borderRadius:18,overflow:"hidden"}}>
                ...
                {(refHistory||[]).length===0 ? (
                  <div style={{textAlign:"center",padding:"40px 20px",color:C.sub}}>
                    ...
                    <div style={{fontSize:14}}>Share your referral link and earn ₦1,000 for each person who subscribes!</div>
                  </div>
                ) : (refHistory||[]).map((r,i)=>(
...
// ANALYTICS PAGE
// ═══════════════════════════════════════════════════════════════
function AnalyticsPage({ isMobile, connected, onConnect, usedActions }) {
  const [range, setRange] = useState("7d");
  const [analyticsData, setAnalyticsData] = useState(null);

  useEffect(() => {
    fetch("https://autoflowng-backend.onrender.com/api/analytics", { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
  ...
  const ACTIONS = analyticsData?.daily || [0, 0, 0, 0, 0, 0, 0];
  const max = Math.max(...ACTIONS, 1);
  const totalActions = analyticsData?.totalActions || usedActions || 0;
  const messagesSent = analyticsData?.messagesSent || 0;
  const leadsCaptures = analyticsData?.leadsCaptures || 0;
  const avgResponse = analyticsData?.avgResponse || "—";

  const connectedPlatforms = (connected||[]);
  const PLATFORM_STATS = connectedPlatforms.length > 0 ? connectedPlatforms.map((p,i) => ({
    id: p.id, name: p.name, color: p.color,
    actions: analyticsData?.byPlatform?.[p.id] || 0,
    pct: analyticsData?.byPlatform?.[p.id] ? Math.round((analyticsData.byPlatform[p.id]/Math.max(totalActions,1))*100) : 0
  })) : [
    ...
    { id:"twitter",   name:"Twitter/X", color:"#1D9BF0", actions:0, pct:0 },
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)", gap:12 }}>
        {[
          { label:"Actions Executed", value:totalActions.toLocaleString(), delta:"", color:C.green },
          { label:"Messages Sent",    value:messagesSent.toLocaleString(), delta:"", color:C.sky },
          { label:"Leads Captured",   value:leadsCaptures.toLocaleString(), delta:"", color:C.amber },
          { label:"Avg Response Time",value:avgResponse, delta:"", color:C.violet },
        ].map(k=>(
      ...
      <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(2,1fr)", gap:14 }}>
        ...
        <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:16, padding:isMobile?"16px":"20px 22px" }}>
          ...
          {[
            { name:"Auto-Reply DMs",     runs:0, rate:"—", color:C.rose },
            { name:"Lead Capture",       runs:0, rate:"—", color:C.green },
            { name:"Email Sequence",     runs:0, rate:"—", color:C.violet },
            { name:"Content Scheduler",  runs:0, rate:"—", color:C.sky },
            { name:"Payment Follow-Up",  runs:0, rate:"—", color:C.amber },
          ].map((a,i)=>(
      ...
      <div style={{ background:`linear-gradient(135deg,rgba(0,200,150,0.07),rgba(56,189,248,0.04))`, border:`1px solid rgba(0,200,150,0.2)`, borderRadius:16, padding:isMobile?"18px":"22px 28px", display:"flex", flexDirection:isMobile?"column":"row", justifyContent:"space-between", alignItems:isMobile?"flex-start":"center", gap:16 }}>
        <div>
          ...
          <div style={{ fontSize:14, color:C.sub, maxWidth:420 }}>Connect your platforms and AutoFlowNG will track all automated actions here. Your time savings will appear once you start running automations.</div>
        </div>
        <div style={{ textAlign:isMobile?"left":"right", flexShrink:0 }}>
...
function BillingPage({ isMobile, onUpgrade, userPlan, payments, usedActions, connectedCount }) {
  const [showUpgrade, setShowUpgrade] = useState(false);
  const currentPlan = (userPlan && userPlan !== "trial") ? userPlan.charAt(0).toUpperCase()+userPlan.slice(1) : "Free Trial";
  const totalActions = userPlan === "starter" ? 1000 : userPlan === "enterprise" ? 999999 : 50000;
  const usedPct = Math.round(((usedActions||0)/totalActions)*100);
  const nextBilling = (userPlan && userPlan !== "trial") ? new Date(Date.now() + 30*24*60*60*1000).toLocaleDateString("en-NG",{month:"long",day:"numeric",year:"numeric"}) : "—";
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:`linear-gradient(135deg,rgba(0,200,150,0.08),rgba(14,165,233,0.05))`, border:`1px solid rgba(0,200,150,0.22)`, borderRadius:18, padding:isMobile?"20px":"26px 28px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)", gap:12 }}>
          {[
            { label:"Actions Used", value:`${(usedActions||0).toLocaleString()} / ${totalActions.toLocaleString()}`, pct:usedPct, color:usedPct>80?C.rose:C.green },
            { label:"Platforms Connected", value:`${connectedCount||0} / All`, pct:Math.round(((connectedCount||0)/10)*100), color:C.sky },
      ...
      <div style={{ background:C.s1, border:`1px solid ${C.border}`, borderRadius:16, overflow:"hidden" }}>
        ...
        {(payments||[]).length === 0 ? (
          <div style={{textAlign:"center",padding:"40px 20px",color:C.sub}}>
            ...
            <div style={{fontSize:14}}>Your payment history will appear here after your first subscription.</div>
          </div>
        ) : (payments||[]).map((h,i)=>(
          <div key={i} style={{ padding:isMobile?"13px 16px":"14px 22px", borderBottom:i<(payments||[]).length-1?`1px solid ${C.border}`:"none", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:isMobile?"wrap":"nowrap" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, minWidth:0 }}>

...
// Default region data (global/USD fallback for unrecognized timezones)
...
function detectRegionData() {
  try {
    ...
    if (prefix === "Africa") return REGION_MAP["Africa/Lagos"]; // fallback African timezones to NGN
    ...
    return DEFAULT_REGION; // USD for anything else
...
function PayModal({ plan, onClose, onSuccess }) {
  ...
  // Ensure Paystack script is loaded before proceeding
...
function KnowledgePage({ isMobile }) {
  ...
  async function send(text) {
    ...
    setLoading(true);
    try {
      const res = await fetch("https://autoflowng-backend.onrender.com/api/knowledge", {
        method:"POST",
        headers:{ "Content-Type":"application/json", "Authorization":`Bearer ${localStorage.getItem("autoflowng_token")||""}` },
        body: JSON.stringify({

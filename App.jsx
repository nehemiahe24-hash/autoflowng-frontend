  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: sz * 0.28, cursor: onClick ? "pointer" : "default", userSelect: "none" }}>
      <svg width={sz} height={sz} viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
        ...
        <rect width="44" height="44" rx="10" fill="#05070F" stroke="rgba(0,200,150,0.25)" strokeWidth="0.6"/>
        {/* Dashed teal orbital ring */}
        <circle cx="22" cy="22" r="14.5" stroke="#00C896" strokeOpacity="0.55" strokeWidth="0.9" strokeDasharray="2.4 2.2" />
        {/* Spokes to nodes */}
        <line x1="22" y1="14.5" x2="22" y2="9" stroke="rgba(255,255,255,0.55)" strokeWidth="0.9" strokeLinecap="round" />
        <line x1="22" y1="29.5" x2="22" y2="35" stroke="rgba(255,255,255,0.55)" strokeWidth="0.9" strokeLinecap="round" />
        <line x1="14.5" y1="22" x2="9" y2="22" stroke="rgba(255,255,255,0.55)" strokeWidth="0.9" strokeLinecap="round" />
        <line x1="29.5" y1="22" x2="35" y2="22" stroke="rgba(255,255,255,0.55)" strokeWidth="0.9" strokeLinecap="round" />
        {/* Central green hub with AF */}
        <circle cx="22" cy="22" r="7.5" fill="url(#grad_logo)" stroke="rgba(255,255,255,0.85)" strokeWidth="0.7"/>
        <text x="22" y="24.6" textAnchor="middle" fontFamily="'Syne',sans-serif" fontWeight="900" fontSize="7.2" fill="#FFFFFF" letterSpacing="-0.04em">AF</text>
        {/* 4 orbital nodes */}
        <circle cx="22" cy="8" r="2.3" fill="#00C896" stroke="rgba(255,255,255,0.9)" strokeWidth="0.5"/>
        <circle cx="36" cy="22" r="2.3" fill="#38BDF8" stroke="rgba(255,255,255,0.9)" strokeWidth="0.5"/>
        <circle cx="22" cy="36" r="2.3" fill="#A78BFA" stroke="rgba(255,255,255,0.9)" strokeWidth="0.5"/>
        <circle cx="8" cy="22" r="2.3" fill="#00C896" stroke="rgba(255,255,255,0.9)" strokeWidth="0.5"/>
        <defs>
...
const TESTIMONIALS = [
  { name: "Daniel Okafor", role: "Small Business Owner", avatar: "D", color: "#00C896", stars: 5, text: "Before using AutoFlowNG, I was spending hours every day replying to customers manually and trying to keep up with follow-ups. After setting everything up, most of those tasks became automatic. It honestly saved me a lot of stress and helped me respond faster to clients. The best part is that it still feels simple to use even if you're not deeply technical." },
  { name: "Blessing Eze", role: "Digital Marketing Consultant", avatar: "B", color: "#38BDF8", stars: 5, text: "We started using AutoFlowNG to automate customer messages and lead tracking for our small business. At first I was skeptical, but after a few weeks I could clearly see the difference. Response time improved, fewer leads were forgotten, and our workflow became more organized. It feels like having an extra team member working in the background 24/7." },
  { name: "Michael Adeyemi", role: "E-commerce Store Manager", avatar: "M", color: "#A78BFA", stars: 5, text: "What I like about AutoFlowNG is that it focuses on practical automation instead of unnecessary complexity. We connected our communication channels and automated repetitive tasks that used to waste time daily. It's not magic overnight success, but it genuinely makes business operations smoother and more efficient." },
];
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.ink, fontFamily:"'DM Sans',sans-serif", overflowX:"hidden" }}>
      ...
      <section id="pricing" style={{padding:mobile?`50px ${px} 60px`:`70px ${px} 80px`,background:C.s1,borderTop:`1px solid ${C.border}`}}>
        <div style={{textAlign:"center",marginBottom:mobile?36:52}}>
          ...
          {isAfrica && <p style={{fontSize:15,color:C.sub,marginTop:4}}>🇳🇬 Nigeria · 🇬🇭 Ghana · 🇰🇪 Kenya · 🇿🇦 South Africa · 🇺🇬 Uganda · 🇹🇿 Tanzania</p>}
        </div>
...
function AppDashboard({ onBack }) {
  ...
  useEffect(() => {
    ...
    .then(d => {
      ...
      setConnectedCount(connList.length);
    })
    .catch(() => {});
        ...
        <div style={{padding:isMobile?"14px 16px":"16px 28px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:C.s1,flexShrink:0,gap:12}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              ...
              {!isMobile&&<div style={{fontSize:15,color:C.sub,marginTop:3}}>Monday, 11 May 2026</div>}
            </div>
          </div>
...
function AnalyticsPage({ isMobile, connected, onConnect }) {
  ...
  const PLATFORM_STATS = [
    { id:"instagram", name:"Instagram", color:"#E1306C", actions:0, pct:72 },
    { id:"whatsapp",  name:"WhatsApp",  color:"#25D366", actions:0, pct:54 },
    { id:"gmail",     name:"Gmail",     color:"#EA4335", actions:0, pct:38 },
    { id:"twitter",   name:"Twitter/X", color:"#000000", actions:0,  pct:26 },
    { id:"linkedin",  name:"LinkedIn",  color:"#0A66C2", actions:0,  pct:14 },
  ];
          ...
          {[
            ...
            <div key={a.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:i<4?`1px solid ${C.border}`:"none" }}>
              <div>
                ...
                <div style={{ fontSize:15, color:C.sub }}>{a.runs.toLocaleString()} runs</div>
              </div>
...
function BillingPage({ isMobile, onUpgrade }) {
  ...
  const currentPlan = trial.subscribed ? "Business" : "Free Trial";
  const nextBilling = trial.subscribed ? "Next month" : "—";
  const usedActions = 0;

...
// ═══════════════════════════════════════════════════════════════
// IMMERSIVE 3D / WebGL EFFECTS  (inline, no extra deps besides three)
//   - GradientMesh: full-bleed animated WebGL shader background
//   - MagneticCursor: dot + ring cursor (desktop only)
//   - ScrollProgress: Linear-style top progress bar
//   - NoiseOverlay: Stripe-style film grain
// To use three.js gradient mesh: npm install three
// ═══════════════════════════════════════════════════════════════
...
function GradientMesh() {
  ...
  useEffect(() => {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader:`
        ...
        void main(){
          ...
          float n2 = snoise(p*2.2 - vec2(t*1.3,t));
          float n3 = snoise(p*0.8 + vec2(-t*0.5,t*1.1));
          vec3 cGreen=vec3(0.0,0.78,0.59), cSky=vec3(0.22,0.74,0.97),
               cViolet=vec3(0.65,0.55,0.98), cBg=vec3(0.015,0.024,0.06);
          vec3 col = cBg;
          col = mix(col,cGreen,  smoothstep(0.0,0.55,n1*0.5+0.5)*0.55);
          col = mix(col,cSky,    smoothstep(0.1,0.7, n2*0.5+0.5)*0.5);
          col = mix(col,cViolet, smoothstep(0.2,0.8, n3*0.5+0.5)*0.4);
          float d = distance(uv,u_mouse);
          col += vec3(0.0,0.45,0.32)*smoothstep(0.45,0.0,d)*0.35;
    ...
    const tick=()=>{ uniforms.u_time.value=(performance.now()-t0)/1000; renderer.render(scene,cam); raf=requestAnimationFrame(tick); };
    tick();
    const onMove=(e)=>{ const r=el.getBoundingClientRect(); uniforms.u_mouse.value.set((e.clientX-r.left)/r.width, 1-(e.clientY-r.top)/r.height); };
    const onResize=()=>{ const w=el.clientWidth,h=el.clientHeight; renderer.setSize(w,h); uniforms.u_res.value.set(w,h); };
    window.addEventListener("mousemove",onMove);
    window.addEventListener("resize",onResize);
    return ()=>{ cancelAnimationFrame(raf); window.removeEventListener("mousemove",onMove); window.removeEventListener("resize",onResize); renderer.dispose(); geo.dispose(); mat.dispose(); if(el.contains(renderer.domElement)) el.removeChild(renderer.domElement); };
  },[]);

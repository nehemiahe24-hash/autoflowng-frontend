...
function AppDashboard({ onBack }) {
  ...
  useEffect(() => {
    ...
    if (!tok) return;
    fetch("https://autoflowng-backend.onrender.com/api/connections", {
    ...
    // Show success toast if Gmail was just connected via OAuth
    const gmailEmail = localStorage.getItem("autoflowng_gmail_just_connected");
    if (gmailEmail) {
      localStorage.removeItem("autoflowng_gmail_just_connected");
      setTimeout(() => toast(`Gmail connected${gmailEmail !== "true" ? ": " + gmailEmail : ""} ✓`, C.green), 600);
    }
  }, []);
...
export default function Root() {
  ...
  useEffect(() => {
    ...
    if (token) {
      // Save JWT token (from login redirect or Gmail callback)

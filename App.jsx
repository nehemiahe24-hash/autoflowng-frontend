...
function WireSphere3D({ size=520 }) {
  ...
  useEffect(() => {
    ...
    const loop=()=>{
      ...
      ry += 0.008; rx = 0.35 + my*0.4 + Math.sin(scrollY*0.001)*0.1;
      const yawAdd = mx*0.8 + scrollY*0.0015;
      const cy=Math.cos(ry+yawAdd),sy=Math.sin(ry+yawAdd),cx=Math.cos(rx),sx=Math.sin(rx);
      ...
      // edges along longitudes
      ctx.lineWidth=0.9;
      for(let i=0;i<LAT;i++){
        for(let j=0;j<LON;j++){
          const a=proj[i*LON+j], b=proj[i*LON+((j+1)%LON)], c=proj[(i+1)*LON+j];
          const za=(a[2]+b[2])/2, zc=(a[2]+c[2])/2;
          const ga=Math.max(0,Math.min(1,(za+R)/(2*R)));
          const gc=Math.max(0,Math.min(1,(zc+R)/(2*R)));
          ctx.strokeStyle=`rgba(0,200,150,${0.15+ga*0.55})`;
          ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(b[0],b[1]);ctx.stroke();
          ctx.strokeStyle=`rgba(56,189,248,${0.10+gc*0.5})`;
          ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(c[0],c[1]);ctx.stroke();
        }
      }
      // glowing points
      for(const p of proj){
        const g=Math.max(0,Math.min(1,(p[2]+R)/(2*R)));
        if(g<0.5)continue;
        ctx.fillStyle=`rgba(167,139,250,${g*0.9})`;
        ctx.beginPath();ctx.arc(p[0],p[1],1.4*g,0,7);ctx.fill();
      }
  ...
  return <canvas ref={ref} style={{width:size,height:size,display:"block",filter:"drop-shadow(0 0 60px rgba(0,200,150,0.35))"}}/>;
}
